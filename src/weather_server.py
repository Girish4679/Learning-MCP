...
def get_weather(location: str) -> dict:
    ...
    params = {
        ...
        'units': 'imperial' # Use 'imperial' for Farenheight, 'metric' for Celcius
    }
    ...
    try:
        ...
        return {
            ...
            'temperature_farenheight': f'{temperature}°F',
            'feels_like_farenheight': f'{feels_like}°F',
            'humidity': f'{humidity}%',
        ...
        if response.status_code == 404:
            ...
            return {"error": f'An HTTP error occured {http_err}'}
...
def compare_weather_prompt(location_a: str, location_b: str) -> str:
    """
    Generate a clear, comparative summary of the weather between two specified locations.
    ...
    The user wants to compare the weather between "{location_a}" and "{location_b}".locals
...
def delivery_log_resource() -> list[str]:
    ...
    """
    try:
        log_file = Path('../logs/delivery_log.txt')
